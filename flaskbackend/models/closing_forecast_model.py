...
def print_metrics_report(
    ...
    if not metrics:
        print("\n⚠ metrics 정보가 없습니다. 모델을 다시 학습(train_prophet_models)해야 할 수 있습니다.")
        return
...
def train_prophet_models(test_horizon: int = 6) -> Dict[str, Any]:
    ...
    for target in TARGET_COLS:
        ...
        print(
            f"✅ [{target}] MAE={best_metrics['MAE']:.0f}, "
    ...
    print(f"\n💾 Prophet 모델 payload 저장 완료: {MODEL_PATH}")

    print_metrics_report(metrics)

    print("\n📊 시나리오용 비용 비율 (최근 기간 기준)")
...
if __name__ == "__main__":
    ...
    print("\n🎉 여기까지 잘 나오면 Flask API에서 그대로 사용하면 됩니다.")
